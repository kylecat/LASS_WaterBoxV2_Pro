...
void loop(void) {
  ...
  digitalWrite(sensorSwitch, LOW);
  EC_mV = _getVoltage(EC_pin);// 只有電壓值
  EC_value =getECmV(EC_mV);
  LWatchDog.feed();

  digitalWrite(sensorSwitch, HIGH);
  //  pH_value = getPH();
  Tubidity_value = _getVoltage(pH_pin);
  LWatchDog.feed();

  String msgTemp = " T:\t" + String(Temp_value, 2) + " C";
  String msgEC = "EC(Raw):" + String(EC_value, 2);
  String msgTb = "Tub(Raw):" + String(Tubidity_value, 2);

  OLED_smallContent(msgTb, msgEC, msgTemp, 2.0);

  Serial.println("[MCU] " + msgTemp);
  Serial.println("[MCU] " + msgTb);
  ...
  CSV_Data = str_Time + "," + String(Temp_value) + "," + String(EC_value) + "," + String(Tubidity_value);
  ...
  msgTb = "F3," + String(Tubidity_value, 2);
  String msgDATE = "F4," + (String)_year + "-" + convert_2digits(_month) + "-" + convert_2digits(_day);
  String msgTIME = "F5," + convert_2digits(_hour) + ":" + convert_2digits(_minute);
