...
/***** << AtlasScientific library >> *****/
//#define AtlasAddr 0x64               //default I2C ID number for EZO EC Circuit.
//#define AtlasAddr 0x63               //default I2C ID number for EZO pH Circuit.
//#define AtlasAddr 0x62               //default I2C ID number for EZO ORP Circuit.
#define AtlasAddr 0x61               //default I2C ID number for EZO DO Circuit.
//#define AtlasAddr 0x66               //default I2C ID number for EZO Temp Circuit.
//#define AtlasAddr 0x67               //default I2C ID number for EZO PMP Circuit.

uint16_t AtlasRespTime = 300;
...
/***** << Atlas function >> *****
   Atlas 模組用
   AtlasSend(String _cmd)  // 寫入指令
   AtlasRead()             // 讀取資料
   getDo                   // 讀取DO
***********************************/
void AtlasSend(String _cmd)
{
  byte code = 0;

  _cmd.toLowerCase();
  if (_cmd.indexOf('r') == 0 || _cmd.indexOf("cal") == 0) AtlasRespTime = 600;
  else                                                    AtlasRespTime = 300;

  Wire.beginTransmission(AtlasAddr);                                      //call the circuit by its ID number.
  Wire.write(_cmd.c_str());                                             //transmit the command that was sent through the serial port.
  Wire.endTransmission();                                               //end the I2C data transmission.

  delay(AtlasRespTime);
  Wire.requestFrom(AtlasAddr, 20, 1); //call the circuit and request 20 bytes (this may be more than we need)
...
float getDo(float _temp = 25.0)
{
  String setTemp = "T," + String(_temp, 2);
  AtlasSend(setTemp);
  AtlasSend("R");
  return AtlasRead();
}
...
void loop(void) {
  ...
  DO_value = getDo(Temp_value);
  LWatchDog.feed();

  String msgTemp = " T:" + String(Temp_value, 2) + " C";
  String msgEC = "EC:" + String(EC_value, 2) + " uS";
  String msgDO = "DO:" + String(DO_value, 2) + " mg/L";

  OLED_smallContent(msgDO, msgEC, msgTemp, 2.0);

  Serial.println(msgTemp);
  Serial.println(msgEC);
  Serial.println(msgDO);

  OLED_smallContent("RTC", "GetDateNow", "Analysis Mode", 0.5, false);
  /*****<< 取得時間資料 >>*****/
  ...
  OLED_smallContent("SD", "Saving", "Analysis Mode", 0.5, false);
  ...
  CSV_Header   = "Date,Temp,pH,DO";
  CSV_Data = str_Time + "," + String(Temp_value) + "," + String(pH_value) + "," + String(DO_value);
  SavingData(CSV_fileName, CSV_Data);     //  寫入CSV
  LWatchDog.feed();

  OLED_smallContent("Data", "UpdateNow", "Analysis Mode", 0.5, false);
  msgTemp = "F1," + String(Temp_value, 2);
  msgEC = "F2," + String(EC_value, 2);
  msgDO = "F3," + String(DO_value, 2);
  String msgCMD = "SLEEP";
